"""
加载 checkpoint 中的逃脱者网络可视化；猎人用 `rule_action_hunter`。

默认：环境由 `configs/default.yaml`（可用 `--config` 换）决定，非 checkpoint 内嵌 cfg；
`seed 0`、`max-episodes 10`、录制 `runs/viz_escaper_policy.mp4`。关闭录屏：`--no-record-mp4`。

  pip install -e ".[viz]"
  python scripts/viz_escaper_policy.py --checkpoint runs/escaper_rl.pt
"""
...
from hunt_env.cli_defaults import (
    DEFAULT_CONFIG_YAML,
    VIZ_MAX_EPISODES,
    VIZ_MP4_ESCAPER_POLICY,
    VIZ_SEED,
    env_cfg_for_viz,
)
from hunt_env.env.hunt_parallel import HuntParallelEnv
from hunt_env.policies.rules import rule_action_hunter
...
# 易被误当成权重的文本后缀（torch.load 会报 UnpicklingError）
...
def _load_checkpoint(path: Path) -> dict:
    ...
    if suf in _CKPT_REJECT_SUFFIXES:
        raise SystemExit(
            ...
            "请使用训练脚本保存的 .pt，例如："
            "`python scripts/train_escaper_ppo_rule_hunter.py --save runs/escaper_rl.pt ...`\n"
            "不要把训练日志 .log 当作 checkpoint。"
        )
    try:
        return torch.load(path, map_location="cpu", weights_only=False)
    except TypeError:
        pass  # 旧版 PyTorch 无 weights_only 参数
...
def _checkpoint_load_failed(path: Path, err: BaseException) -> None:
    raise SystemExit(
        ...
        f"原因：{err}\n"
        "请确认路径为 PyTorch 保存的 .pt/.pth（含 state_dicts），而不是日志或其它文本。"
    ) from None


def main() -> None:
    p = argparse.ArgumentParser(
        description="逃脱者策略可视化：从 .pt 加载网络，与规则猎人同场"
    )
    p.add_argument(
        "--checkpoint",
        type=str,
        required=True,
        help="含 state_dicts['escaper'] 的 checkpoint（预训练或 PPO 存盘）",
    )
    p.add_argument(
        "--config",
        type=str,
    ...
    p.add_argument(
        ...
        type=float,
        default=10.0,
        help="与 EscaperPretrainConfig.obs_clip 对齐；归一化 obs 的 clip 范围",
    )
    p.add_argument(
        "--record-mp4",
        type=str,
        default=VIZ_MP4_ESCAPER_POLICY,
        help="MP4 输出路径（默认 runs/viz_escaper_policy.mp4）",
    )
    p.add_argument("--no-record-mp4", action="store_true", help="只预览，不写 MP4")
    p.add_argument(
        "--record-max-frames",
        type=int,
        default=None,
    ...
    if "escaper" not in sds:
        raise SystemExit("checkpoint 中无 state_dicts['escaper']")
    sd_e = sds["escaper"]
    ...
    obs_dim_net, hidden = _infer_backbone(sd_e)
    ...
    pol.load_state_dict(sd_e)
    pol.eval()

    obs_rms: RunningMeanStd | None = None
    orm = ck.get("obs_rms")
    if isinstance(orm, dict) and "escaper" in orm:
        st = orm["escaper"]
    ...
    try:
        ...
        print(
            f"猎人: rule_action_hunter（与预训练/PPO 规则对手一致）。"
        ...
        while running:
            ...
            if any(terms.values()) or any(truncs.values()):
                ...
                er = rews.get(ek, 0.0) if ek else 0.0
                print(f"局结束：{reason}，示例奖励 {ek or 'escaper'}={er:.2f}")
