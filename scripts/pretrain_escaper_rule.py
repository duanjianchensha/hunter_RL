#!/usr/bin/env python3
"""
逃脱者预训练：并行环境中猎人用规则，仅优化逃脱者共享 ActorCritic。

- 行为克隆：策略头均值监督为规则逃脱动作经仿射后的单位盒坐标（与 PPO `act` 一致）；
- 价值头：在规则猎人 rollout 上用 GAE 回报回归。

权重与 escaper 的 obs_rms（及可选 rew_rms）存于单独目录；并行 env 数以 YAML 为准（可用 `--num-envs` 覆盖）；后续 PPO 可用 `--init-escaper`。

  pip install -e ".[rl]"
  python scripts/pretrain_escaper_rule.py --config configs/default.yaml --out-dir pretrained/escaper_rule
  python scripts/pretrain_escaper_rule.py ... --log-file pretrained/escaper_rule/train.log
"""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="规则猎人示教下并行预训练逃脱者 ActorCritic（BC + 价值 GAE）"
    )
    p.add_argument("--config", type=str, default=DEFAULT_CONFIG_YAML)
    p.add_argument(
        "--out-dir",
        type=str,
        default="pretrained/escaper_rule",
        help="只写该目录，不污染其它 checkpoint；内含 escaper.pt 与 run.json",
    )
    p.add_argument(
        "--total-env-steps",
        type=int,
    ...
    with tee_stdout_stderr(args.log_file):
        _pretrain_escaper_rule_run(args)


def _pretrain_escaper_rule_run(args: Namespace) -> None:
    if args.device == "auto":
    ...
    (out / "README.txt").write_text(
        "本目录为逃脱者规则预训练 checkpoint，与主训练保存路径分离；PPO 请用 "
        "python scripts/train_escaper_ppo_rule_hunter.py --init-escaper 指向 escaper.pt\n",
        encoding="utf-8",
    )

    env = HuntVectorizedEnv.from_yaml(args.config, num_envs=args.num_envs)
    pre = EscaperPretrainConfig(
        lr=args.lr,
    ...
    trainer = EscaperRulePretrainer(env.cfg, env, pre_cfg=pre, device=device)
    print(
    ...
    meta = {
        "script": "pretrain_escaper_rule.py",
    ...
    while total < args.total_env_steps:
        ...
        if upd % args.print_every == 0 or total >= args.total_env_steps:
            print(
                f"env_steps {total}/{args.total_env_steps} | e_rew_mean={m_coll['escaper_rew_mean']:.4f} | "
                f"bc={tstats['bc']:.4f} v={tstats['v']:.4f} loss={tstats['loss']:.4f}"
            )

    path_pt = out / "escaper.pt"
    meta_out = {
        **meta,
    ...
    (out / "run.json").write_text(
        json.dumps(
            {**meta_out, "escaper_pt": str(path_pt.resolve())},
            indent=2,
            ensure_ascii=False,
        ),
        encoding="utf-8",
